...
- **device_tools**：设备 CLI 结果 helper，统一 `list-devices`、`activate-devices`、`test` 的 JSON envelope。

消息格式统一使用 forge_msgs 的 `RobotState`、`RobotAction`。

设备工具 JSON 统一包含 `ok`、`capability`、`supported`、`message`。`list-devices` 在 `devices` 中返回统一 address 对象 `{ name, address, status }`（可选 `role`）。不适用于某个机器人时命令仍返回 `ok: true, supported: false`，便于前端统一调用。
