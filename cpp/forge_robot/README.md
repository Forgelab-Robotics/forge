...
`v1.0.0` with CXX bridge sources, so the default build path is pinned to that
...
Build and compile-link validate the Dora runner against Dora `v1.0.0`:
...
The command above downloads the SHA256-pinned Dora `v1.0.0` source archive and
builds `dora-node-api-cxx` with Cargo. To avoid downloading during
configure/build, provide a local Dora `v1.0.0` checkout:

```bash
git clone --branch v1.0.0 https://github.com/dora-rs/dora.git /path/to/dora-v1.0.0
cmake -S cpp/forge_robot -B cpp/forge_robot/build-dora \
  -DFORGE_ROBOT_CPP_WITH_DORA=ON \
  -DDORA_ROOT_DIR=/path/to/dora-v1.0.0
cmake --build cpp/forge_robot/build-dora --target forge_robot_dora_runner_compile
```
...
`FORGE_ROBOT_CPP_WITH_DORA=ON`. The default path builds against Dora `v1.0.0`;
pass `DORA_ROOT_DIR=/path/to/dora-v1.0.0` to use an existing local checkout.
